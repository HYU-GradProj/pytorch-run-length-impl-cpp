...
class AllreduceRing : public Algorithm {
 ...
 protected:
  ...
  void compress_outbox()
  {
    int compressed_len = compress<T>(outbox_, count_);
    sendDataBuf_->change_send_len(compressed_len);
  }

  void compress_inbox()
  {
    int compressed_len = compress<T>(inbox_, count_);
    sendDataBuf_->change_send_len(compressed_len);
  }

  void decompress_inbox()
  {
    decompress<T>(inbox_, count_);
  }
};
