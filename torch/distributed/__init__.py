...
import random
import statistics
def all_reduce(tensor, op=reduce_op.SUM, group=group.WORLD, threshold=False):
    """Reduces the tensor data across all machines in such a way that all get
    the final result.
    ...
    """
    if (threshold):
        threshold_candidate = random.sample(list(tensor), int(tensor.numel() * 0.001)) 
        threshold_median = statistics.median(threshold_candidate)

        tensor[tensor <= threshold_median] = 0

    assert torch.distributed._initialized == _INITIALIZED_PG, \
        "collective only supported in process-group mode"
    return torch._C._dist_all_reduce(tensor, op, group, threshold)
